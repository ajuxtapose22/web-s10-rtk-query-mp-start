...
const [deleteQuote, { isLoading: deletingQuote }] = useDeleteQuoteMutation()
  ...
  return (
    <div id="quotes">
      <h3>Quotes
      {deletingQuote && <span className="loading-message deleting"> in deletion...</span>}
      {togglingQuote && <span className="loading-message toggling"> being toggled...</span>}
      {(gettingQuotes || refreshingQuotes) && <span className="loading-message loading"> loading...</span>}
    </h3>
      <div>
        {
          quotes?.filter(qt => {
